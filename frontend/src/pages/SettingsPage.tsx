...
export default function SettingsPage() {
  ...
  const themes = [
    ...
    return <div
  className="h-max container mx-auto gap-2 max-w-[1366px]  px-4 pt-20 overflow-auto "
  style={{
  ...
  >
    <div className='space-y-6'>
        ...
        {themes.map((themes, i) => (
            ...
            >
              <div className="h-8   relative w-full ">
                <div data-theme={themes} className='absolute inset-0 grid grid-cols-4 gap-1 p-1 rounded-md'>
      ...
      <div className=" mt-3  rounded-lg md:border md:border-base-300 shadow-2xl bg-none md:bg-base-300 mb-5">
                ...
                >
                  {PREVIEW_MESSAGES.map((message,index
                      ...
                      className={`flex ${message.isSent ? "justify-end" : "justify-start"}`}>
                          <div className={
                              `rounded-md p-2 sm:p-4  ${message.isSent ? " bg-primary " : "bg-secondary"}`
                          }>
                              <p className={ `${message.isSent ? "text-primary-content" : "text-secondary-content"}`}>{message.content}</p>
                              <p className={`items-start text-sm text-base-content/60 ${message.isSent ? "text-primary-content" : "text-secondary-content"}`}>12.pm</p>
                          </div>
                      </div>
                  ))}
