    ...
    (set, get) => ({
      ...
      sendFriendRequest: async (to: string, message?: string) => {
        try {
          ...
          const errorMessage =
            (error as AxiosError<{ message: string }>)?.response?.data
              ?.message || "Đã xảy ra lỗi khi gửi yêu cầu kết bạn";
          toast.error(errorMessage);
          throw error;
        } finally {
          set({ loading: false });
      ...
      acceptFriendRequest: async (requestID: string) => {
        try {
          ...
          const errorMessage =
            (error as AxiosError<{ message: string }>)?.response?.data
              ?.message || "Đã xảy ra lỗi khi chấp nhận yêu cầu kết bạn";
          toast.error(errorMessage);
          throw error;
        } finally {
          set({ loading: false });
      ...
      rejectFriendRequest: async (requestID: string) => {
        try {
          ...
          const errorMessage =
            (error as AxiosError<{ message: string }>)?.response?.data
              ?.message || "Đã xảy ra lỗi khi từ chối yêu cầu kết bạn";
          toast.error(errorMessage);
          throw error;
        } finally {
          set({ loading: false });
      ...
      getAllFriendRequest: async () => {
        try {
          ...
          const errorMessage =
            (error as AxiosError<{ message: string }>)?.response?.data
              ?.message || "Đã xảy ra lỗi khi lấy yêu cầu kết bạn";
          toast.error(errorMessage);
          throw error;
        } finally {
          set({ loading: false });
